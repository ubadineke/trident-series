...
impl FuzzTest {
    ...
    fn start(&mut self) {
        ...
        let vault = self.fuzz_accounts.vault.insert(&mut self.trident, Some(PdaSeeds { seeds: &[b"vault"], program_id: vulnerable::program_id()}));
        let user = self.fuzz_accounts.user.insert(&mut self.trident, None);
        let user_account = self.fuzz_accounts.user_account.insert(&mut self.trident, Some(PdaSeeds { seeds: &[b"user", user.as_ref()], program_id: vulnerable::program_id()}));
    ...
    fn flow2(&mut self) {
        ...
        if result.is_success(){
            let account_data = self.trident.get_account_with_type::<UserAccount>(&user_account, 8).unwrap();
             assert!(
                account_data.balance < 1_000_000_000,
                "Error, Balance overflowed to {}",
                account_data.balance
            )
        }
        // if let Some(account_data) = self.trident.get_account_with_type::<UserAccount>(&user_account, 8)
        // {
        //     assert!(
        //         account_data.balance < 1_000_000_000_000,
        //         "Error, Balance overflowed to {}",
        //         account_data.balance
        //     )
        // }
